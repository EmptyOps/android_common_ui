...
import com.tvc_reservation_management.android_common_ui.R;
...
public class RatingView extends View {
    ...
    private void parseAttrs(AttributeSet attrs) {
        ...
        borderColor = arr.getColor(R.styleable.SimpleRatingBar_srb_borderColor, ContextCompat.getColor(getContext(), R.color.colorAccent));
    ...
    private void validateAttrs() {
        ...
        if (numberOfStars <= 0) {
            throw new IllegalArgumentException(String.format("RatingView initialized with invalid value for numberOfStars. Found %d, but should be greater than 0", numberOfStars));
        }
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
            Log.w("RatingView", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
            throw new IllegalArgumentException(String.format("RatingView initialized with invalid value for stepSize. Found %f, but should be greater than 0", stepSize));
        }
        if (starBorderWidth <= 0) {
            throw new IllegalArgumentException(String.format("RatingView initialized with invalid value for starBorderWidth. Found %f, but should be greater than 0",
                    starBorderWidth));
        }
        if (starCornerRadius < 0) {
            throw new IllegalArgumentException(String.format("RatingView initialized with invalid value for starCornerRadius. Found %f, but should be greater or equal than 0",
                    starBorderWidth));
        }
    }
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
            Log.w("RatingView", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new Animator.AnimatorListener() {
            @Override
            ...
            public void onAnimationEnd(Animator animator) {
                if (ratingListener != null) {
                    ratingListener.onRatingChanged(RatingView.this, rating, false);
                }
            }
    ...
    public interface OnRatingBarChangeListener {
         ...
         */
        void onRatingChanged(RatingView simpleRatingBar, float rating, boolean fromUser);
    ...
    public class AnimationBuilder {
        ...
        private AnimationBuilder(RatingView ratingBar) {
